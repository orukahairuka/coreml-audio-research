...
Phase 1 で `normal_loud` 以外が出た組合せに対し、以下 9 戦略の効果率を測る。

| ID | 戦略 | 実装の要点 |
|---|---|---|
| A | dummy warm-up 1 回 → 本番 | `loadModels` 直後に zero 入力で 1 回 synthesize。本番は wav を採用 |
| B | dummy warm-up 2 回 → 本番 | A の dummy を 2 連続 |
| C | warm-up → 検査 → retry | dummy 後 / 本番後の rms/NaN/Inf を見て異常なら再合成（最大 N 回） |
| D | Decoder のみ短い dummy で warm-up | Encoder/HiFi-GAN は skip し Decoder model だけ短メル長で 1 回 predict |
| E | Encoder / Decoder / HiFi-GAN を個別 warm-up | 3 段独立に warm-up し、どの段の warm-up が効くかを切り分け |
| F | cpuOnly で warm-up → cpuAndGPU で本番 | `MLModelConfiguration` を 2 つ作り、warm-up 用と本番用で切替 |
| G | インスタンス再生成 vs 使い回し | 各 iter で `AudioSynthesizer` を再 init するパターンを別途取る |
| H | fixed262 vs RangeDim 比較 | `ShapeModeOption` を切り替えた Repeat3 を取る |
| I | 別 precision/engine を先に走らせた後に対象を走らせる | プロセス全体ウォーム状態の影響を測る |

各戦略について **N=20 試行** を取り、`normal_loud` の出現率（％）を集計する。
