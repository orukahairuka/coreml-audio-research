...
- **22050Hz に変換する理由**: PronounSE のモデルが 22050Hz のデータで学習されているため、入力を同じサンプルレートに揃える必要がある。異なるサンプルレート（例: 44100Hz）のまま渡すと、モデルにとって異なる速度の音声として認識され、正しい合成結果が得られない
- **モノラルに変換する理由**: PronounSE のモデルがモノラル（1チャンネル）の音声で学習されているため。ステレオ（2チャンネル）は左右で異なる音を持てるが、効果音合成では左右の定位は不要なのでモノラルで十分
- **PCM (Pulse Code Modulation)**: 音声をデジタルデータにする最も基本的な形式。アナログ波形を一定間隔（22050Hz なら1秒に22050回）でサンプリングして数値化したもの。wav ファイルの中身はこの PCM データであり、`AVAudioPCMBuffer` は PCM 形式の音声データを格納するバッファ
- **ポインタ経由で配列変換する理由**: `AVAudioPCMBuffer` は C/Objective-C 時代から続くオーディオフレームワークで、パフォーマンス重視でメモリをポインタで直接管理している。大量の音声サンプル（数万〜数十万）を高速に処理するための設計であり、Swift の `[Float]` 配列として扱うにはポインタ → `UnsafeBufferPointer` → `Array` の変換が必要
- **バッファではなく `[Float]` 配列で返す理由**: `AVAudioPCMBuffer` は AVFoundation 専用の型なので、そのまま返すと以降の処理も AVFoundation に依存してしまう。`[Float]` に変換して返すことで、後続の処理（プリエンファシス、STFT など）は AVFoundation を知らなくて済む

#### (2) preprocess(_:)

入力音声の先頭の無音を切り取り、末尾をフェードアウトする。録音の「余分な無音」と「終端のブツ切れ」をきれいにするための前処理。

- **オンセット検出**: 音が始まる位置を見つける処理。窓内のエネルギー合計が閾値（0.08）を超えたら「ここから音が始まっている」と判定し、それ以前の無音をカットする
  - **エネルギー**: 音の大きさを数値で表したもの。各サンプルを二乗して合計する。無音なら値がほぼ 0、音があれば大きくなる
  - **窓**: 波形の一部分を切り出す範囲。1024 サンプル（22050Hz で約 46ms）分のエネルギーを測り、256 サンプルずつずらして走査する。これらの値は STFT のパラメータ（nFFT=1024, hopLength=256）と揃えている
- **フェードアウト**: 末尾 15% の区間で音量を徐々にゼロに下げ、音がブツッと切れるのを防ぐ
- **パラメータの値**: Python 版 `PronounSE/Transformer/utils.py` の `preprocess()` のデフォルト引数と同じ値を使っている。学習時にこの値で前処理されているため、推論時も合わせる必要がある

| パラメータ | 値 | 意味 |
|---|---|---|
| `onsetThreshold` | 0.08 | 音の開始を判定するエネルギー閾値 |
| `onsetShift` | 256 | オンセット検出時に窓をずらす幅（サンプル数） |
| `onsetWindowLength` | 1024 | エネルギーを測る窓の幅（サンプル数） |
| `fadeOutRatio` | 0.15 | フェードアウトする割合（末尾 15%） |

#### (3) applyPreemphasis(_:)

- 高周波を強調するフィルタ（プリエンファシスフィルタ）
- `y[n] = x[n] - 0.97 * x[n-1]`
- 音声信号は低周波成分が支配的で高周波が埋もれやすいため、前処理で高周波を持ち上げて音を聞き取りやすくし、モデルの学習・推論精度を上げる
